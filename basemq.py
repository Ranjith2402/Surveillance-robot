...
class BaseMQ(object):
    ...
    # This strategy measure values immediately, so it might be inaccurate. Should be
    ...
    #  @param baseVoltage Optionally board could run on 3.3 Volts, base voltage is 5.0 Volts. Passing incorrect values
    #  would cause incorrect measurements
    #  @param measuringStrategy Currently two main strategies are implemented:
    #  - STRATEGY_FAST = 1 In this case data would be taken immediately. Could be unreliable
    ...
    #  received from previous runs After calibration is completed @see _ro attribute could be stored for
    ...
    def heatingCompleted(self):
        if (self._heater) and (not self._cooler) and (
                utime.ticks_diff(utime.ticks_ms(), self._prMillis) > self.MQ_HEATING_PERIOD):
            return True
        else:
            return False
    ...
    def coolanceCompleted(self):
        if (self._heater) and (self._cooler) and (
                utime.ticks_diff(utime.ticks_ms(), self._prMillis) > self.MQ_COOLING_PERIOD):
            return True
        else:
            return False
