...
const char* password = "dica6245";

bool is_AutoMode = false;
  ...
  <body class="noselect" align="center" style="background-color:white">
    <table id="mainTable" style="width:400px;margin:auto;table-layout:fixed" CELLSPACING=10>
      <tr>
        ...
        <tr>
            <td></td>
            <td class="switch_button" ontouchend="changeLabelText()">
                <label id="myLabel" class="myLabel">Manual</label>
            </td>
        </tr>
      <tr>
        <td></td>
    ...
    <script>
      ...
      function changeLabelText() {
        ...
        if (myLabel.innerHTML === "Auto") {
          myLabel.innerHTML = "Manual";
        }
        else {
          myLabel.innerHTML = "Auto";
        }
        sendButtonInput("Mode", myLabel.innerHTML)
      }
                      ...
                      size_t len)
{
  ...
  {
    case WS_EVT_CONNECT:
      Serial.printf("WebSocketClientConnected #%u from %s", client->id(), client->remoteIP().toString().c_str());
      break;
    case WS_EVT_DISCONNECT:
      Serial.printf("WebSocketClientDisconnected #%u", client->id());
      moveCar(0);
...
//        Serial.printf("Key [%s] Value[%s]\n", key.c_str(), value.c_str());
        ...
        else if (key == "Tilt")
        {
          ...
          is_AutoMode = !is_AutoMode;
          Serial.print("MODE:");
          this is an intentional error cuz boolean is no printing
          Serial.println(String(is_AutoMode));
        }
      }
      break;
    case WS_EVT_PONG:
...
{
  read_serial();
  WiFiClient client = app_server.available();
  if (client){
    if (client.available()){
      from_client = client.readStringUntil('\n');
      Serial.print("CLIENT:");
      Serial.println(from_client);
    }
    if (send_to_client){
      client.println(to_client);
